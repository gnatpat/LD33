package net.natpat 
{
	...
	public class GV 
	{
		...
		{
			tracker.trackPageview("themonsterfiles/" + page);
		}
		
		public static var currentEpisode:int;
		...
		private static function shakeUpdate(e:Event):void
		{
			...
			offset.x = (Math.random() * intensity - (intensity / 2)) * Ease.quartOut(duration / totalDuration);
			offset.y = (Math.random() * intensity - (intensity / 2)) * Ease.quartOut(duration / totalDuration);
			camera.x += offset.x;
			camera.y += offset.y;
		}
		...
		public static function freeze(delay:Number, duration:Number):void
		{
		...
		public static function freezeNow(duration:Number):void
		{
			frozen = true;
			var freezeTimer:Timer = new Timer(duration, 1);
			freezeTimer.addEventListener(TimerEvent.TIMER, function():void {
				frozen = false;
			});
			freezeTimer.start();
		}
